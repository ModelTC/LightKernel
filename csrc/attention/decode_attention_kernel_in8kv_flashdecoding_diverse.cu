...
template<
    ...
    typename T>
__global__
void dynamic_batching_flashdecoding_cache_attention_int8kv_diverse_kernel(
    ...
    const int32_t * __restrict__ b_shared_seq_len,
    ...
    // 对于 float (4 bytes): VEC_SIZE = 4; 对于 half (2 bytes): VEC_SIZE = 8
    ...
    // 线程 ID 映射
    const int64_t warp_id       = threadIdx.x / WARP_SIZE;          // 当前线程所属的 warp ID
    const int64_t warp_lane_id  = threadIdx.x % WARP_SIZE;          // 当前线程在 warp 内的 lane ID
    const int64_t group_id      = warp_lane_id / THREAD_GROUP_SIZE; // 当前线程所属的线程组 ID (在 warp 内)
    const int64_t group_lane_id = warp_lane_id % THREAD_GROUP_SIZE; // 当前线程在线程组内的 lane ID
    ...
    // 加载 Query 到寄存器
    // 线程组协作加载：每个线程加载 VEC_LEN 个向量，每个向量 VEC_SIZE 个元素
    // 内存访问模式：strided access，步长为 THREAD_GROUP_SIZE * VEC_SIZE
    // 这样可以让线程组内的线程访问连续的内存区域，提高合并访问效率
    #pragma unroll
    for (int64_t i = 0; i < VEC_LEN; i++) {
        // copy 128(16 * 8) bits from Q to Local Q

        // 这个地方是错开间隔读取的，不知道如果设置成为连续位置读取会不会一样呢？
        // 使用 128-bit 向量化拷贝 (16 bytes = VEC_SIZE * sizeof(T))
        // 访问模式示例 (THREAD_GROUP_SIZE=8, VEC_SIZE=8):
        //   thread 0: elements [0:8],   [64:72]
        //   thread 1: elements [8:16],  [72:80]
        //   ...
        //   thread 7: elements [56:64], [120:128]
        // 看起来每个thread读取到的q值，只和group_lane_id有关，也就是只有 0 - 7
        // 总共128个值，每个读取16。毕竟是由VEC_LEN = (HEAD_SIZE / VEC_SIZE) / THREAD_GROUP_SIZE计算出来的
        // 那么会有 256 个线程，一个组里面8个线程，一共会读取 32 个组读取 32 遍？哦可以 因为都需要Q
    ...
    // ------------------------------------------------ //
    // Step 2. Solve QK Dot
    // 使用 shared memory 存储 attention scores
    // 大小为 max(seq_block_size, head_size) * sizeof(float)
    extern __shared__ float logits[];

    // 在线 softmax：跟踪当前 block 内的最大值，用于数值稳定的 softmax
    float qk_max = -FLT_MAX;

    // 遍历所有 tokens，计算 Q·K^T
    // 每个线程组处理一个token，GPT 32 个线程组并行处理256个token，循环8次
    // base_id = warp_id * GPW + group_id
    for (int64_t base_id = warp_id * GPW; base_id < context_len; base_id += GPT) {
        ...
        if (context_id >= context_len){
            ...
            // 从 KV cache 加载 K (INT8)
            const int64_t key_offset
                            = (mem_context_id) * kcache_stride_s  // token 位置
                            + kv_head_idx * kcache_stride_h       // KV head 位置
                            + group_lane_id * VEC_SIZE;           // 线程组内偏移
            #pragma unroll
            for (int64_t i = 0; i < VEC_LEN; i++) {
    ...
    // ------------------------------------------------ //
    // Step 3. Softmax
    // Shared memory 用于 block 内的 reduce 操作
    __shared__ float red_smem[WPT];

    // Block 内 reduce：找到所有线程的 qk_max 的最大值
    // 这是在线 softmax 的关键步骤，用于数值稳定性
    // WPT 8个warp
    qk_max = attn_block_reduce_max<WPT>(qk_max, red_smem);

    // 计算 exp(x - max) 并累加，得到归一化因子
    float exp_sum = 0.0f;
...
template<typename T>
void run_group_int8kv_decode_flashattention_diverse_kernel(
    const int64_t seq_block_size, 
    T* __restrict__ output_emb,
    T* __restrict__ output_logexpsum,       
    ...
    if (reduce_shm_size + logits_size <= MAX_SHM_SIZE) {
        ...
        switch (head_dim){
            case 64:
                dynamic_batching_flashdecoding_cache_attention_int8kv_diverse_kernel<64, 4, 256, 8>
                ...
                (
                    ...
                    b_seq_len, b_shared_seq_len, b_req_idx, req_to_tokens,
                ...
                break;
            case 96:
                dynamic_batching_flashdecoding_cache_attention_int8kv_diverse_kernel<96, 4, 256, 8>
                <<<grid_size, 256, logits_size, stream>>>
                (
                    seq_block_size,
                    output_emb,
                ...
                break;
            case 128:
                dynamic_batching_flashdecoding_cache_attention_int8kv_diverse_kernel<128, 8, 256, 8>
                <<<grid_size, 256, logits_size, stream>>>
                (
                    seq_block_size,
                    output_emb,
                ...
                break;
            case 256:
                dynamic_batching_flashdecoding_cache_attention_int8kv_diverse_kernel<256, 16, 256, 8>
                <<<grid_size, 256, logits_size, stream>>>
                (
                    seq_block_size,
                    output_emb,
...
void group_int8kv_flashdecoding_attention_diverse(const int seq_block_size, at::Tensor mid_o_emb, at::Tensor mid_o_logexpsum, float att_scale, at::Tensor q, at::Tensor k, at::Tensor k_s,  at::Tensor v,  at::Tensor v_s, at::Tensor req_to_tokens, at::Tensor b_req_idx, at::Tensor b_seq_len, at::Tensor b_shared_seq_len, int max_len_in_batch) {
    ...
    LIGHT_DISPATCH_FLOATING_TYPES(q.scalar_type(), "group_int8kv_flashdecoding_attention_diverse", ([&] {
        run_group_int8kv_decode_flashattention_diverse_kernel<scalar_t>(
            seq_block_size, 
            ...
            b_shared_seq_len.data_ptr<int32_t>(),
...
void group_int8kv_flashdecoding_attention_diverse(
    ...
    torch::Tensor b_shared_seq_len,
    int64_t max_len_in_batch)
{
    group_int8kv_flashdecoding_attention_diverse(
