...
// CUDA kernel for per token quantization from FP16 to INT8
template<int32_t TPB>
__global__ void device_per_token_quant_fp16_to_int8_general(
...
// CUDA kernel for per token quantization from FP16 to INT8
template<int32_t TPB>
__global__ void device_per_token_quant_fp16_to_int8_vpt(
    const fp16_t* __restrict__ input,  // Input tensor in FP16 format
    int8_t* __restrict__ output,       // Output tensor in INT8 format
    fp32_t* __restrict__ scales,       // Output scales for each token
    const int32_t N
    ...
    for (int32_t i = tid * VPT; i < N; i += TPB * VPT) {
        ...
        #pragma unroll
        for (int32_t j = 0; j < VPT / 2; j++) {
            fp32x2_t x = fp16x2_to_fp32x2(local_fp16[j]);
...
__global__ void device_per_token_quant_fp16_to_int8(
    const fp16_t* __restrict__ input,  // Input tensor in FP16 format
    int8_t* __restrict__ output,       // Output tensor in INT8 format
    fp32_t* __restrict__ scales        // Output scales for each token
) {
...
void per_token_quant_fp16_int8(
    ...
    switch (N) {
        case 16:
            device_per_token_quant_fp16_to_int8<128, 16>
            ...
            break;
        case 32:
            device_per_token_quant_fp16_to_int8<128, 32>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 64:
            device_per_token_quant_fp16_to_int8<128, 64>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 512:
            device_per_token_quant_fp16_to_int8<128, 512>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 1024:
            device_per_token_quant_fp16_to_int8<128, 1024>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 3200:
            device_per_token_quant_fp16_to_int8<128, 3200>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 4096:
            device_per_token_quant_fp16_to_int8<128, 4096>
            <<<blocks, 128, 0, at::cuda::getCurrentCUDAStream()>>>(
                PTR<fp16_t>(contiguous_input),
                PTR<int8_t>(output),
                PTR<fp32_t>(contiguous_scales)
            ...
            break;
        case 12800:
            device_per_token_quant_fp16_to_int8<256, 12800>
        ...
        default: {
            ...
            if (N % 8 == 0) {
                device_per_token_quant_fp16_to_int8_vpt<TPB>
                ...
                device_per_token_quant_fp16_to_int8_general<TPB>
                <<<blocks, TPB, shared_mem_size, at::cuda::getCurrentCUDAStream()>>>(
                    PTR<fp16_t>(contiguous_input),
                    PTR<int8_t>(output),
                    PTR<fp32_t>(contiguous_scales),
